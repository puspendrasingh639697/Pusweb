...
//             <div className={`mb-3 transition-colors ${item.isFeatured ? 'text-[#f8a548]' : 'text-[#f8a548] group-hover:text-white'}`}>
//               {item.icon}
//             </div>
...
export default function PyramidGrid() {
    ...
    <div className={`flex justify-center gap-4 ${rowNum > 1 ? '-mt-14' : ''} flex-wrap`}>
      {features.filter(f => f.row === rowNum).map((item, idx) => (
        ...
        <motion.div
          ...
          className="relative w-64 h-72 flex items-center justify-center group transition-all duration-300 [transform-style:preserve-3d]"
          style={{
            filter:
              "drop-shadow(0px 25px 35px rgba(0,0,0,0.18)) drop-shadow(0px 5px 10px rgba(0,0,0,0.08))"
          }}
        >

          {/* HEXAGON BACKGROUND */}
          <div
            className={`absolute inset-0 transition-all duration-500 border-2 group-hover:-translate-y-2
            ${item.isFeatured
                ...
                : 'bg-white border-zinc-200 group-hover:bg-[radial-gradient(ellipse_farthest-corner_at_right_bottom,#FEDB37_0%,#FDB931_8%,#9f7928_30%,#8A6E2F_40%,transparent_80%),radial-gradient(ellipse_farthest-corner_at_left_top,#FFFFFF_0%,#FFFFAC_8%,#D1B464_25%,#5d4a1f_62.5%,#5d4a1f_100%)]'
              }`}
            style={{
              clipPath: "polygon(25% 0%, 75% 0%, 100% 50%, 75% 100%, 25% 100%, 0% 50%)"
            }}
  ...
  return (
    <section className="bg-slate-50 py-24 px-6 min-h-screen flex flex-col items-center justify-center [perspective:1200px]">
      <div className="max-w-7xl w-full">
        {[1, 2, 3, 4].map(num => renderRow(num))}
      </div>
    </section>
